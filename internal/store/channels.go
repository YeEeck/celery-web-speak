...
SELECT id, type, name, COALESCE(audio_bitrate_kbps, 0), COALESCE(message_retention, 0), created_at, updated_at
...
INSERT INTO channels (type, name, message_retention, created_at, updated_at) VALUES (?, ?, 500, ?, ?)`,
		...
		result, err = tx.ExecContext(ctx, `
INSERT INTO channels (type, name, audio_bitrate_kbps, created_at, updated_at) VALUES (?, ?, 64, ?, ?)`,
			channelType, name, formatTime(now), formatTime(now))
	}
	if err != nil {
...
func (s *Store) UpdateChannel(ctx context.Context, actorID, channelID int64, name string, audioBitrateKbps, messageRetention int) (Channel, error) {
	name = strings.TrimSpace(name)
	if err := validateChannelName(name); err != nil {
		return Channel{}, err
	}
	...
	} else if audioBitrateKbps < 32 || audioBitrateKbps > 128 || audioBitrateKbps%8 != 0 {
	...
	if channel.Type == ChannelTypeText {
		...
		_, err = tx.ExecContext(ctx, `UPDATE channels SET name = ?, audio_bitrate_kbps = ?, updated_at = ? WHERE id = ?`,
			name, audioBitrateKbps, formatTime(s.now()), channelID)
	}
	if err != nil {
		if isUniqueError(err) {
			return Channel{}, ErrChannelNameExists
	...
	details := fmt.Sprintf("channel_id=%d type=%s name=%q bitrate=%d retention=%d", channelID, channel.Type, name, audioBitrateKbps, messageRetention)
...
func scanChannel(scanner channelScanner) (Channel, error) {
	...
	if err := scanner.Scan(&channel.ID, &channel.Type, &channel.Name, &channel.AudioBitrateKbps, &channel.MessageRetention, &createdAt, &updatedAt); err != nil {
		return Channel{}, err
	}
