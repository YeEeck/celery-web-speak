...
- `voice`：LiveKit Room、音频设备、联动的麦克风/耳机静音状态、参与者状态和本地音量；LiveKit 初次连接最多重试 5 次，之后使用 SDK 自动重连。
...
业务 WebSocket 广播以下事件：在线状态、消息创建/删除、频道设置更新、用户资料或权限更新、用户删除、会话撤销。服务端每 25 秒发送 Ping，读超时为 60 秒，单客户端发送队列容量为 64；队列已满时跳过该次广播，连接错误或会话撤销时关闭连接。
...
- 语音禁言会同时更新 SQLite 状态与 LiveKit 发布权限；临时封禁、永久封禁、管理员重置密码和账号删除会撤销会话并从房间移除参与者。
