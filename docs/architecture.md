...
业务 WebSocket 广播以下事件：在线状态、频道创建/更新/删除、带频道 ID 的消息创建/删除与设置更新、语音房间成员快照、用户资料或权限更新、用户删除、会话撤销。频道全部公开，因此频道业务事件仍可向所有已登录连接广播。服务端每 25 秒发送 Ping，读超时为 60 秒，单客户端发送队列容量为 64；队列已满时跳过该次广播，连接错误或会话撤销时关闭连接。
