...
> 目标实现基线：v0.2.0。系统面向单机、单 Go 进程和单 LiveKit 节点部署，不具备水平扩展的一致性设计。
...
从单频道版本升级时保留 `users`、`sessions`、`invites`、`temporary_bans` 和 `audit_logs`，丢弃旧 `messages` 与单例 `settings`，创建默认公开文字频道“文字聊天”和语音频道“语音频道”。迁移使用显式 schema 版本，避免依赖 `CREATE TABLE IF NOT EXISTS` 猜测结构。
