...
业务 WebSocket 广播以下事件：在线状态、频道创建/更新/删除、带频道 ID 的消息创建/删除与设置更新、语音房间成员快照、用户资料或权限更新、用户删除、会话撤销。频道全部公开，因此频道业务事件仍可向所有已登录连接广播。
...
普通业务事件使用容量为 64 的有界发送队列。在线状态使用容量为 1 的独立最新值缓冲：新完整快照覆盖尚未发送的旧快照，不受普通事件积压影响。Hub 在连接增减时实时广播完整在线账号 ID，并每 15 秒无条件重发当前完整快照，使已建立但曾错过单次广播的客户端自动纠正。该周期广播只校准 Go 进程内的业务连接状态，与 LiveKit 语音房间校准彼此独立。
...
- 全量校准失败时保留最后一次有效快照并记录警告；校准成功后仅在快照变化时广播。周期任务不会重叠执行，发现同一账号残留在多个房间时保留加入时间较新的连接并移除旧连接。
- 同一账号在所有房间中只允许一个语音连接。最新签发的频道令牌决定目标房间；加入新房间时移除旧房间参与者，过期目标或旧令牌产生的加入会被服务端移除。
...
- 启动数据按文字频道计算未读数量。当前频道到达底部后提交最后已读消息 ID；消息因保留策略被删除不回退已读 ID。
