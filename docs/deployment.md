...
当前稳定版本为 `v0.3.7`。生产环境应让仓库标签、部署文件和 `APP_IMAGE` 使用同一版本，不要长期跟随 `latest`。
...
更新前先完成 SQLite 数据备份，并确认目标版本的发布说明。将下面的 `v0.3.7` 替换为要升级到的版本：

```bash
git fetch --depth 1 origin tag v0.3.7
git checkout v0.3.7
```
