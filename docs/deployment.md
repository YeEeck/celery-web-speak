...
当前稳定版本为 `v0.4.4`。生产环境应让仓库标签、部署文件和 `APP_IMAGE` 使用同一版本，不要长期跟随 `latest`。
...
更新前先完成 SQLite 数据备份，并确认目标版本的发布说明。以下示例将服务更新到当前稳定版本 `v0.4.4`：

```bash
git fetch --depth 1 origin tag v0.4.4
git checkout v0.4.4
```
