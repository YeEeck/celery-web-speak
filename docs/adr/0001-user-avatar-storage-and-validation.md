...
为支持用户自定义头像,在 `users` 表新增 `avatar_version`(单调计数,上传与删除各 +1,不归零)、`avatar_bytes BLOB NULL`、`avatar_mime TEXT NULL`。头像字节直接 BLOB 存入 SQLite,沿用单一二进制部署与 `SetMaxOpenConns(1)` 的串行写入模型。后端用 `image.DecodeConfig` 解析图头校验尺寸 ≤1024×1024、宽高比 1:1 容差 <0.02、MIME ∈ {image/png,image/jpeg,image/webp,image/gif},但**不二次编码**;校验按用途参数化,GIF 仅头像白名单放行且放宽 1:1(旁路裁剪器后无方形保证,见 ADR-0028);前端对 PNG/JPEG/WebP 输出 512×512 方形 WebP(q≈85),GIF 跳过裁剪器原字节直传。
...
- **2026-08-06(GIF 动图头像)**:MIME 白名单新增 `image/gif`(仅头像,校验按用途参数化,服务器图标不变)、GIF 放宽 1:1 并旁路客户端裁剪器原字节直传、上传总量上限 4MB→8MB;详见 ADR-0028。