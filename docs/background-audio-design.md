...
| 编码 | Opus、立体声、独立背景音码率（64-256 kbps，默认 128）、关闭 DTX 与语音处理，RED 默认关闭且可按频道开启 |
| 发送音量 | 0%-100%，默认 50%，当前客户端本地持久化 |
| 听众音量 | 分享者发送增益 x 听众用户音量 x 听众总输出音量 |
| 权限 | 所有未被语音禁言的成员均可使用 |
| 远端状态 | 会话存在期间常显音乐图标，检测到真实声音亮起，持续约 5 秒无声音熄灭变灰（暂停同为灰，不隐藏）；收听者本地静音该背景音时音符变黄替代亮起/变灰，取消后恢复；停止共享后隐藏。见 ADR-0029、ADR-0030 |
...
- 背景音按钮位于频道栏底部的纯语音工具栏右侧操作组，与麦克风、耳机静音和断开语音并列；账户头像、设置和退出登录不属于该工具栏。
...
控制面板在背景音播放和暂停期间都必须保留上述控件，不能因原生状态快照与 Web 发布流程的到达顺序而退化为仅显示标题。底部背景音按钮在实际播放时使用活动色，并在悬停和键盘聚焦期间保持活动色；暂停后仍可打开控制面板继续播放或停止，但不使用播放中的活动色。

发送音量默认 50%，保存在当前客户端的 `localStorage`。发送音量只改变发布到频道的增益，不改变目标应用在分享者电脑上的本地播放音量。0% 保持共享会话和音轨存在，但发送静音信号。

### 远端展示与音量

- 背景音会话存在期间，成员列表在发送者旁常显音乐图标；检测到真实声音亮起（绿色），持续约 5 秒无声音熄灭变灰，暂停同样为灰；收听者本地静音该背景音时音符变黄，取消后恢复（ADR-0029、ADR-0030）。
- 发送者停止共享（publication 取消）后不显示图标。
- 不显示目标程序、窗口或媒体名称。
- 听众按发送者分别保存麦克风音量和背景音音量，两者相互独立且范围均为 0%-300%，默认均为 100%。
- 发送者存在背景音 publication 时显示独立背景音音量控件，播放中和暂停时均保留；publication 取消后隐藏控件，但保留该发送者的本地设置。
- 听众实际背景音增益为分享者发送增益、该发送者背景音音量和听众总输出音量的乘积，客户端播放增益最高限制为 300%。
...
- RED 丢包冗余按频道独立配置，默认关闭；管理员开启后应结合真实丢包测试评估连续性收益与额外带宽。

背景音使用独立的频道级码率配置 `backgroundAudioBitrateKbps`（64-256 kbps，步进 16，默认 128），与语音码率 `audioBitrateKbps`（32-128 kbps）分离。管理员可在频道设置中独立调整。

### 订阅与播放

客户端附加所有远端音频轨，并按来源分别应用本地音量：

- 明确识别 `screen_share_audio` publication。
- 把同一用户保存的麦克风音量应用到 `microphone`，把独立保存的背景音音量应用到 `screen_share_audio`。
- 耳机静音与总输出音量同时作用于两类轨道。
- 背景音会话存在期间音乐图标常显，亮/灰由接收端对背景音轨道的真实声音分析决定；暂停（publication muted）不隐藏，静默约 5 秒后熄灭变灰；收听者本地静音时音符变黄（替代亮/灰），取消后恢复（ADR-0029、ADR-0030）。
- 背景音 publication 存在但已 muted 时继续显示背景音音量控件。
...
- 远端音乐图标在 screen_share_audio publication 存在期间常显，亮/灰由接收端真实声音分析决定（ADR-0029）。
- 按用户独立麦克风与背景音音量分别作用于对应音轨，并与总输出音量叠加。
- 背景音不触发麦克风说话高亮。
- 频道背景音码率或背景音 RED 配置变化时，背景音使用新参数重新发布。
...
- 听众按用户独立麦克风与背景音音量及总输出音量正确作用于对应音轨。
