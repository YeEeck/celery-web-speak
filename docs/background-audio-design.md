...
| 权限 | 所有未被语音禁言的成员均可使用 |
| 远端状态 | 会话存在期间常显音乐图标，检测到真实声音亮起，持续约 5 秒无声音熄灭变灰（暂停同为灰，不隐藏）；停止共享后隐藏。见 ADR-0029 |
...
- 背景音会话存在期间，成员列表在发送者旁常显音乐图标；检测到真实声音亮起（绿色），持续约 5 秒无声音熄灭变灰，暂停同样为灰（ADR-0029）。
...
- 耳机静音与总输出音量同时作用于两类轨道。
- 背景音会话存在期间音乐图标常显，亮/灰由接收端对背景音轨道的真实声音分析决定；暂停（publication muted）不隐藏，静默约 5 秒后熄灭变灰（ADR-0029）。
