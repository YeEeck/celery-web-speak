...
3. 将按钮从固定宽度改为内容自适应宽度，设置约 `8px` 横向内边距和约 `44px` 可点击高度，并将可见交互背景纵向收紧到约 `34px`；右侧按钮组继续独立靠右。
4. 静止态不显示背景，精确指针 hover、键盘聚焦、按下和切换中使用中性的交互反馈；触摸设备不保留粘滞 hover。
...
1. 在语音连接摘要中保存已连接频道的 `audioBitrateKbps`，加入时取频道配置，缺失时回退到 `64`。
2. 频道设置热更新时同步刷新码率快照；切换浏览服务器和重连时保留，离开语音时清空。
3. 将连接信息第二行调整为可省略的 `服务器名 / 频道名` 和不可收缩的 `· 64 kbps` 两部分。
4. 码率沿用次级文字样式，不增加强调色、标签或 tooltip。
5. 覆盖默认值、真实连接、切换服务器、重连状态和长名称窄屏布局，确认码率始终完整且面板高度稳定。

### 验证

- `npm run typecheck`
- Playwright 定向运行语音连接摘要和窄屏布局用例。
- 桌面、移动端和 320px 窄视口截图检查。
- `npm run build`
- `go test ./...`
...
- 连接信息显示已连接频道的配置发送码率，跨服务器浏览和重连时保持正确，长名称不会挤掉码率。
