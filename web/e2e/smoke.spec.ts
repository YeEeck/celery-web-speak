...
test('语音工具栏按职责分栏并持久化 DTX 模式', async ({ page, isMobile }) => {
  ...
  await expect(toolbar.getByTitle('麦克风静音')).toBeEnabled()
  await expect(toolbar.getByTitle('耳机静音')).toBeEnabled()
...
test('静音时音量滑块保留数值并弱化当前无效的控制', async ({ page, isMobile }) => {
  ...
  await toolbar.getByTitle('麦克风静音').hover()
  ...
  await toolbar.getByTitle('麦克风静音').click()
  ...
  await toolbar.getByTitle('耳机静音').hover()
  ...
  await toolbar.getByTitle('耳机静音').click()
  ...
  await toolbar.getByTitle('取消耳机静音').hover()
  await toolbar.getByTitle('取消静音').hover()
  await expect(microphonePopover).toHaveClass(/is-muted/)
...
test('音频处理与静音说话提醒开关持久化到浏览器', async ({ page, isMobile }) => {
  ...
  await expect(noiseSelect).toHaveValue('webrtc')
  await expect.poll(() => page.evaluate(() => ({
...
test('头像菜单集中账户操作且退出登录使用明确确认', async ({ page, isMobile }) => {
  ...
  const autoItem = menu.getByRole('menuitem', { name: '自动模式', exact: true })
  ...
  await expect(fixedAwayItem).toBeFocused()
  await page.keyboard.press('ArrowDown')
