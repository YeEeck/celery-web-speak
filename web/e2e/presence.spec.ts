...
test('业务连接恢复后同步断线期间的频道和消息', async ({ isMobile, page, request }) => {
  ...
  try {
    ...
    await expect.poll(async () => {
      const bootstrap = await (await request.get(`/api/guilds/${guildID}/bootstrap`)).json() as { online: { userId: number; client: string }[] }
      return bootstrap.online.some((entry) => entry.userId === accountID)
    }).toBe(false)
    const channelResponse = await request.post(`/api/guilds/${guildID}/channels`, { data: { type: 'text', name: channelName } })
    expect(channelResponse.ok()).toBeTruthy()
    channelID = (await channelResponse.json() as { channel: { id: number } }).channel.id
    const messageResponse = await request.post(`/api/guilds/${guildID}/channels/${channelID}/messages`, { data: { content: message } })
    ...
    if (channelID) await request.delete(`/api/guilds/${guildID}/channels/${channelID}`)
    await deletePlatformUser(request, accountID, account.username)
  }
})
