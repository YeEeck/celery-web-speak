...
当前稳定版本为 `v0.4.23`，生产部署建议固定使用明确版本的预构建镜像。
...
用户设置面板分为四个页签：账号、音频、音效与主题。音频、主题和提示音的开关/音量/来源选择保存在当前浏览器的 `localStorage`，自定义音效的音频文件保存在当前浏览器的 IndexedDB；两者都不写入服务器、不同步到其他浏览器，并即时生效。账号修改（显示名称、密码）使用各自独立的保存按钮。
...
```bash
git clone --branch v0.4.23 --depth 1 https://github.com/YeEeck/celery-web-speak.git
cd celery-web-speak
```
...
APP_IMAGE=ghcr.io/yeeeck/celery-web-speak:v0.4.23
