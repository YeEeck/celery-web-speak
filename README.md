...
- 麦克风、扬声器及按用户独立音量支持 0%-300% 增益
- 语音成员按角色与本次加入时间稳定排序，并显示当前说话人
- 加入语音、退出语音和新文字消息操作提示音
- 最近 N 条纯文字消息、游标分页、虚拟列表与实时在线状态
...
用户设置中的麦克风增益、扬声器音量、按用户音量和操作提示音设置均立即生效，并保存在当前浏览器的 `localStorage`，不会写入服务器或同步到其他浏览器。

操作提示音默认开启，默认音量为 60%。用户可以关闭全部提示音，也可以分别控制加入语音、退出语音和新文字消息三类提示：
...
在完整 Compose 环境中设置 `E2E_LIVEKIT=1`，还会创建两个临时账号，验证双方订阅远端音频轨道、成员稳定排序、麦克风与耳机静音状态同步、按用户音量以及加入和退出提示音。
