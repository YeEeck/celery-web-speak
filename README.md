...
当前稳定版本为 `v0.4.18`，生产部署建议固定使用明确版本的预构建镜像。
...
- 回声抑制与降噪开关可随时调整，更改在下次加入语音频道时生效。自动增益控制保持关闭，不暴露开关。
...
```bash
git clone --branch v0.4.18 --depth 1 https://github.com/YeEeck/celery-web-speak.git
cd celery-web-speak
```
...
APP_IMAGE=ghcr.io/yeeeck/celery-web-speak:v0.4.18
