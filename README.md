...
当前稳定版本为 `v0.4.38`，生产部署建议固定使用明确版本的预构建镜像。
...
- 服务器所有者、服务器管理员、普通成员，以及服务器级语音/文字禁言和临时/永久封禁
- 平台管理员可停用、重置或不可恢复地删除全局账号；历史消息匿名保留，原登录名可重新使用
- 游戏内语音浮层（桌面客户端）：透明置顶穿透窗口展示语音参与者状态，大小、位置与两档不透明度可在设置面板"语音浮层"页配置，仅桌面客户端提供

完整范围与权限矩阵见 [产品规格](docs/product-spec.md)，服务关系见 [技术架构](docs/architecture.md)，版本发布流程见 [发版流程](docs/release-process.md)。

## 客户端音频设置

用户设置面板分为五个页签：账号、音频、音效、主题与语音浮层（仅桌面客户端显示）。音频、主题、提示音和浮层配置的开关/音量/来源/大小位置/不透明度保存在当前浏览器的 `localStorage`，自定义音效的音频文件保存在当前浏览器的 IndexedDB；两者都不写入服务器、不同步到其他浏览器，并即时生效。账号修改（显示名称、密码）使用各自独立的保存按钮。

### 音频

- 麦克风与扬声器可在加入语音频道前预选，连接后也可即时切换。麦克风增益与扬声器音量支持 0%-300% 调节，始终可配置。
- 回声抑制开关可随时调整，更改在下次加入语音频道时生效。降噪选项（关闭 / 系统降噪 / 增强降噪）默认增强降噪，切换即时生效；增强降噪由客户端 RNNoise 管线承担（48kHz 音频上下文），加载失败或环境不支持时本次会话静默回退到系统降噪，设置值不变、下次仍尝试。自动增益控制在采集端默认开启以稳定发送电平，不暴露开关。
...
操作提示音默认开启，默认音量为 60%。用户可以关闭全部提示音，也可以分别控制加入语音、退出语音和新文字消息三类提示。每个事件可以从系统预置音效中选用不同的提示音，或为本浏览器上传一个自定义音效文件作为该事件的来源；可试听当前选中的来源。

- 自定义音效支持 MP3、WAV、OGG、M4A 和 WEBM 格式，单文件不超过 512 KB、时长不超过 3 秒。原始文件保存在当前浏览器的 IndexedDB，不上传服务器、不同步到其他浏览器。每个事件的自定义音效独占一个槽位，不跨事件复用；删除某事件的自定义音效后，该事件回退到上一次使用的系统预置。
- 自己成功加入语音频道时播放一次加入音效；主动断开、切换频道、退出登录或离开当前服务器时播放一次退出音效。被动断开和耳机静音时不播放自己的退出音效。
- 其他用户在初始成员同步完成后加入或退出时播放对应音效；初次进入频道和网络重连不会为已有成员逐个提示。
- 当前正在查看的文字频道收到其他用户消息时播放消息音效，自己发送的消息不提示；未进入的文字频道只增加未读数量。
- 静音说话提醒不属于操作提示音，使用固定双音、不接受预置选择或自定义上传。
- 耳机静音会同时静音语音和操作提示音；设置面板中的试听不受耳机静音影响。提示音优先跟随已选择的输出设备，不支持指定输出设备的浏览器会使用系统默认设备。
- 同类提示音在 300ms 内最多播放一次，避免批量上下线或连续消息形成叠音；试听不受该限流约束。
...
```bash
git clone --branch v0.4.38 --depth 1 https://github.com/YeEeck/celery-web-speak.git
cd celery-web-speak
```
...
APP_IMAGE=ghcr.io/yeeeck/celery-web-speak:v0.4.38
