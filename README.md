...
当前稳定版本为 `v0.4.34`，生产部署建议固定使用明确版本的预构建镜像。
...
- 耳机静音会同时静音语音、操作提示音和试听。提示音优先跟随已选择的输出设备，不支持指定输出设备的浏览器会使用系统默认设备。
...
```bash
git clone --branch v0.4.34 --depth 1 https://github.com/YeEeck/celery-web-speak.git
cd celery-web-speak
```
...
APP_IMAGE=ghcr.io/yeeeck/celery-web-speak:v0.4.34
