...
当前稳定版本为 `v0.4.13`，生产部署建议固定使用明确版本的预构建镜像。
...
- `v0.4.5` 为聊天历史增加按用户本地自然日显示的日期分隔线。
- `v0.4.6` 在用户主动离开语音时为本人播放退出提示音，并增强 WebSocket 重同步时的状态版本保护。
- `v0.4.7` 优化语音快捷控制，支持悬停调节音量、右键快速选择设备，并允许在连接语音前预设静音、耳机静音、DTX 与首选设备。
- `v0.4.8` 统一 API、WebSocket 协议与前端的 Guild 领域术语。
...
- `v0.4.13` 精简服务器标题栏，增加频道右键菜单，并在保留文本编辑、链接与选区操作的前提下抑制无意义的浏览器默认菜单。
...
```bash
git clone --branch v0.4.13 --depth 1 https://github.com/YeEeck/celery-web-speak.git
cd celery-web-speak
```
...
APP_IMAGE=ghcr.io/yeeeck/celery-web-speak:v0.4.13
